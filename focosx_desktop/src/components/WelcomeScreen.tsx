...
export const WelcomeScreen: React.FC<WelcomeScreenProps> = ({ onOpenVault, onOpenDocs }) => {
  ...
  // use shared hook for detection
  // Note: import is relative to this file
  // We'll lazily import the hook to avoid build-time resolution issues.
  React.useEffect(() => {
    let mounted = true;
    (async () => {
      try {
        const mod = await import('../hooks/useTauri');
        const available = await mod.waitForTauri(3000);
        if (mounted) setIsTauri(available);
      } catch (e) {
        if (mounted) setIsTauri(false);
      }
    })();
    return () => {
      mounted = false;
  ...
  React.useEffect(() => {
    // Wait until we have a definitive true/false decision, then load vaults.
    if (isTauri !== null) loadVaults();
  }, [isTauri]);
