...
const App: React.FC = () => {
  ...
  const { fileTree, createNode, deleteNode, getNode, updateTree: _updateTree, reloadTree: _reloadTree, isRealFs: _isRealFs } =
    useFileSystem(currentVault?.id || null, currentVault);
  ...
  const handleUploadFile = async (
    ...
    const isText = file.name.match(
      /\.(txt|md|json|js|ts|tsx|html|css|py|csv|svg|canvas)$/i,
    );
    ...
    <div className="flex h-screen w-screen bg-background text-text font-sans overflow-hidden relative">
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto px-2 pb-4 no-scrollbar">
          <div className="text-xs font-bold text-zinc-600 uppercase tracking-wider mb-2 mt-2 px-2">
            Explorer
          </div>
          <FileExplorer
            nodes={fileTree}
