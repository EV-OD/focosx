...
/**
 * Wait for the Tauri runtime to be injected into `window.__TAURI__`.
 * Resolves `true` if available within the timeout, otherwise `false`.
 */
...
/**
 * React hook to detect Tauri availability.
 * Returns `null` while checking, `true` if available, or `false` if not.
 */
